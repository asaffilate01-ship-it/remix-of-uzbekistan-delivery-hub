...
const DEFAULT_INPUTS: Inputs = {
  riders: 3000,
  deliveriesPerDay: 16,
  revenuePerDelivery: 1.1,
  riderPayout: 0.7,
  bikeLease: 12,
  maintenancePerRider: 30,
  overheadMonthly: 175000,
};
...
export function FinancialsModel({ initial }: { initial?: Partial<Inputs> }) {
  ...
  const costBreakdown = [
    { name: "Rider payouts", value: Math.round(m.payouts) },
    { name: "Maintenance & fuel", value: Math.round(m.maintenance) },
      ...
      <div className="glass rounded-2xl p-6 md:p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Drag the sliders — every figure on this page recalculates instantly.</p>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <SliderRow label="Revenue / delivery" value={i.revenuePerDelivery * 100} min={50} max={250} step={5}
            onChange={(v) => setI({ ...i, revenuePerDelivery: v / 100 })} display={gbp(i.revenuePerDelivery, { maximumFractionDigits: 2 })} />
          <SliderRow label="Rider payout / delivery" value={i.riderPayout * 100} min={30} max={150} step={5}
            onChange={(v) => setI({ ...i, riderPayout: v / 100 })} display={gbp(i.riderPayout, { maximumFractionDigits: 2 })} />
          <SliderRow label="Bike lease / rider / week" value={i.bikeLease} min={0} max={30} step={1}
            onChange={(v) => setI({ ...i, bikeLease: v })} display={gbp(i.bikeLease)} />
          <SliderRow label="Monthly overheads" value={i.overheadMonthly} min={50000} max={500000} step={5000}
            onChange={(v) => setI({ ...i, overheadMonthly: v })} display={gbp(i.overheadMonthly)} />
        </div>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Kpi label="Monthly revenue" value={gbp(m.totalRevenue)} accent="primary" sub={`${num(m.monthlyDeliveries)} deliveries`} />
        <Kpi label="Monthly cost" value={gbp(m.totalCost)} sub={`${num(i.riders)} riders`} />
        <Kpi label="Net profit / month" value={gbp(m.netProfit)} accent="gold" sub={`${m.margin.toFixed(1)}% margin`} />
        <Kpi label="Annualised profit" value={gbp(m.netProfit * 12)} accent="primary" sub="run-rate" />
      </div>
        ...
        <div className="glass rounded-2xl p-6 lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={320}>
            <AreaChart data={projection}>
              ...
              <YAxis stroke="oklch(0.72 0.02 240)" fontSize={12} tickFormatter={(v) => `£${(v / 1000).toFixed(0)}k`} />
          ...
          <div className="space-y-2 mt-2">
            {costBreakdown.map((c, idx) => (
              <div key={c.name} className="flex items-center justify-between text-sm">
                ...
                <span className="font-mono">{gbp(c.value)}</span>
              </div>
            ))}
          </div>
        </div>
...
export { LineChart, Line }; // re-export so other pages can build extra charts cheaply
